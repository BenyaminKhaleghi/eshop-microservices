...
namespace Discount.Grpc.Services
{
    public class DiscountService(DiscountContext dbContext, ILogger<DiscountService> logger) : DiscountProtoService.DiscountProtoServiceBase
    {
        public override async Task<CouponModel> GetDiscount(GetDiscountRequest request, ServerCallContext context)
        {
            ...
            if (coupon == null)
                coupon =  new Coupon { ProductName = "", Amount = 0, Description = "No desc"};
        ...
        public override async Task<DeleteDsicountResponse> DeleteDsicount(DeleteDsicountRequest request, ServerCallContext context)
        {
            var coupon = await dbContext.Coupons.FirstOrDefaultAsync(x => x.ProductName == request.ProductName);
            if (coupon is null)
            ...
            return new DeleteDsicountResponse { Success = true };
        }
    }
}
